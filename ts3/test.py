...
class TS3ProtoTest(unittest.TestCase):
    ...
    def testConstructBasic(self):
        ...
        res = self.ts3.construct_command('testcommand', keys={'key1': 'test', 'key2': 'test'})

        if res != "testcommand key1=test key2=test" and res != "testcommand key2=test key1=test":
            self.assertEqual(res, "testcommand key1=b'test' key2=b'test'")

        res = self.ts3.construct_command('testcommand', keys={'key1': 'test', 'key2': [1, 2, 3]})
        if res != "testcommand key1=test key2=1|key2=2|key2=3" and res != "testcommand key2=1|key2=2|key2=3 key1=test":
            self.assertEqual(res, "testcommand key1=b'test' key2=1|key2=2|key2=3")

        res = self.ts3.construct_command('testcommand', keys={'key1':'test', 'key2': 'test'}, opts=['test'])
        if res != "testcommand key2=test key1=test -test" and res != "testcommand key1=test key2=test -test":
